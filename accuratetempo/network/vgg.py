...
def create_vgg_like_model(input_shape=(40, 256, 1), output_dim=256, filters=64, pool_shape=(2, 2),
                          filter_shapes=[(5, 5), (3, 3)], dropout=0.3):
    """
    Create a VGG-style model according to teh given specs.

    :param input_shape: shape
    :param output_dim: classes
    :param filters: number of filters in the first layers.
    :param pool_shape: max pool shape
    :param filter_shapes: filter shapes
    :param dropout: dropout
    :return: model
    """

    model_name = 'vgg_like_in={}_out={}_filters={}_pool_shape={}_max={}_filter_shapes={}_dropout={}'\
        .format(input_shape, output_dim, filters, pool_shape, True, filter_shapes, dropout)\
        .replace(',', '_').replace(' ', '_')
    ...
    x = possible_pool(1, input_shape, pool_shape, x)
    x = Dropout(dropout)(x)
    ...
    x = possible_pool(2, input_shape, pool_shape, x)
    ...
    x = possible_pool(3, input_shape, pool_shape, x)
    x = Dropout(dropout)(x)
    ...
    x = possible_pool(4, input_shape, pool_shape, x)
    ...
    x = possible_pool(5, input_shape, pool_shape, x)
    x = Dropout(dropout)(x)
    ...
    x = possible_pool(6, input_shape, pool_shape, x)
    x = Dropout(dropout)(x)
...
def possible_pool(layer, input_shape, pool_shape, x):
    """
    Create an appropriate max pool layer, taking remaining width and height into account.

    :param layer: depth
    :param input_shape: input
    :param pool_shape: pool shape
